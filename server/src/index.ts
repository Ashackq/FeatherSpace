...
import type {
  DirectMessage,
  IncomingMessage,
  // FeatherSpace Server: Real-time Collaboration Backend
  //
  // Provides WebSocket-based real-time room, chat, object, and environment sync for the FeatherSpace platform.
  // Manages user presence, state, and message relays for spatial collaboration.
  //
  // Key responsibilities:
  //   - Room/user lifecycle (join, leave, reconnect, cleanup)
  //   - Object and environment state sync
...
const roomEnvironments = new Map<
  ...
  // All state is ephemeral and per-process (no persistence or clustering)

  // roomId -> (userId -> user state)
  string,
  // roomId -> (objectId -> object state)
  {
  // roomId -> chat messages (bounded)
    config: Record<string, unknown>;
  // roomId -> direct messages (bounded)
    updatedAt: number;
  // roomId -> environment config (last update)
    updatedBy: string;
  }
>();
...
app.get("/health", (_req, res) => {
  ...
  // userId -> disconnect timer (for grace period)
...
function safeParse(raw: string): IncomingMessage | null {
  ...
  // --- Utility: Safe JSON parse for incoming messages ---
...
// Broadcast the current state of all users in a room to all clients in that room.
...
// Normalize and sanitize display names (fallback to userId if blank)
...
// Ensure chat message array exists for a room
...
// Send the full chat history for a room to a client
...
// Ensure direct message array exists for a room
...
// Send all direct messages visible to a user in a room
...
// Ensure object state map exists for a room
...
// Send the full object state for a room to a client
...
// Send the current environment config for a room to a client
...
// Cancel any pending disconnect timer for a user (e.g. on reconnect)
...
// Store a user's display name (normalized)
...
// Get a user's display name, falling back to userId if not set
...
wss.on("connection", (socket) => {
  ...
  // --- Heartbeat: mark socket as alive on pong ---
  socket.on("pong", () => {
    socketAlive.set(socket, true);
  });

  // --- Main message handler: all client events ---
  socket.on("message", (buffer) => {
    const message = safeParse(buffer.toString());
    if (!message) return;
    ...
    if (message.type === "join_room") {
      ...
      // If user is switching rooms, remove from previous room and broadcast exit
      const previousInfo = socketToUser.get(socket);
      if (previousInfo && previousInfo.roomId !== message.roomId) {
        const previousRoom = rooms.get(previousInfo.roomId);
        if (previousRoom && previousRoom.has(previousInfo.userId)) {
      ...
      // Add user to new room, preserving state if reconnecting
      const room = rooms.get(message.roomId) ?? new Map<string, UserState>();
      if (!rooms.has(message.roomId)) {
        rooms.set(message.roomId, room);
      }
      ...
      // Send all current state to the new user
      sendObjectStateSnapshot(message.roomId, socket);
      sendEnvironmentState(message.roomId, socket);
      sendRoomChatSnapshot(message.roomId, socket);
      sendDirectMessageSnapshot(message.roomId, message.userId, socket);
    ...
    // --- Object state update (e.g. whiteboard, shared object) ---
    if (message.type === "object_event") {
      const sender = socketToUser.get(socket);
      if (!sender) return;
  ...
  // --- WebSocket close: handle disconnect and cleanup ---
  socket.on("close", () => {
    socketAlive.delete(socket);
    const info = socketToUser.get(socket);
    if (!info) {
  ...
  // --- WebSocket error: log and continue ---
  socket.on("error", (error) => {
    log("ws.connection_error", {
      message: error.message,
    });
...
// --- Heartbeat: Periodically ping all sockets and clean up dead connections ---
